...
\maketitle
\begin{abstract}
    This project investigates the application of modern deep reinforcement learning (RL) algorithms to the classic Snake game, a grid-based environment that requires strategic navigation and long-term planning. We implement and compare value-based methods (DQN, Double DQN) and policy-based methods (A2C, PPO) under several policies (MLP and CNN), and evaluate their performance across different state representations and reward shaping strategies. Our study aims to provide insights into how algorithmic design choices impact learning efficiency and final performance in a constrained, progressively complex RL environment. We also explore the potential of learned policies to guide tree search methods, comparing them against traditional search algorithms like Depth-First Search (DFS) and Breadth-First Search (BFS). The full code is available at \url{https://github.com/JeanJNMV/MDS-RL_Snake_Agent}.
\end{abstract}
...
\section{Methodology}
\paragraph{Policy Architectures} MLP and CNN.

\subsection{Environment Setup and Wrapper}
\subsection{Agents}
...
\clearpage 
{\LARGE PROPOSAL (to be removed in the final report)}
