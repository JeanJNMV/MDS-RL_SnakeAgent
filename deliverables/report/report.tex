...
Several works have applied deep reinforcement learning to the Snake game.
Wei et al.~\cite{wei2018autonomous} trained a CNN-based DQN agent on the
classic Snake game, proposing a proximity-based reward mechanism and a
dual experience replay strategy to handle sparse and delayed rewards.
Sebastianelli et al.~\cite{sebastianelli2021deep} explored DQN with
hand-crafted sensor features rather than image pixels, showing that
compact state representations can achieve competitive performance with
significantly reduced training time.
In contrast to these works, our study goes beyond the classic single-food
setting by introducing multiple food types, poison, and dynamic obstacles,
and provides a systematic comparison of hand-crafted MLP features against
egocentric and full-grid CNN representations across environments of
...
capturing local spatial structure;\\
...
random, and dynamic obstacles). The hard setting introduces non-stationarity,
partial observability under egocentric views, and long-horizon planning
challenges.
% ── 3. Methods ────────────────────────────────────────────────────────────────
\section{Methods}
\label{sec:methods}
...
for dynamic obstacles and mitigates partial observability.
...
training, all curves are smoothed using an exponential moving average. All the runs were performed on a NVIDIA A100 GPU cluster.
